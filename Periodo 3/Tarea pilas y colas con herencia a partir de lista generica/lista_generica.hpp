...
// lista_generica
//
// Creado por Fernando Lopez 04/11/2017
//
...
// Contructor con tamanio por defecto, en este caso 4
template <typename TIPODATO>
...
Lista<TIPODATO>::Lista(const Lista<TIPODATO> &otra){
	...
	for (int i = 0; i < this->cuenta; i++)
		this->items[i] = otra.items[i];                           //Posible error
}
...
void Lista<TIPODATO>::insertar(int indice, TIPODATO item) {
	...
	for (int i= cuenta-1; i>=indice; i--){
		cout << "Desplazando elento " << items[i] << "del indice";
